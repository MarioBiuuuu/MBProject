...
#import "NSString+MBDBHandle.h"

NSString *MBDBDefaultName = @"MBDB_default.sqlite";

#define DBParentPrefix @"MBDBParentID_"
#define DBChildPrefix  @"MBDBChildID_"
#define kPId  @"__pid__"
#define kCId  @"__cid__"

#ifdef DEBUG
#ifdef MBDBBUG
#define MBDBLog(fmt, ...) NSLog((@"%s [Line %d]\n" fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
#else
#define MBDBLog(...)
#endif
#else
...
static int MBDBBusyHandler(void *f, int count);
...
- (instancetype)init {
    return [self initWithPath:[MBDB defaultDbPath]];
}
...
+ (NSString *)defaultDbPath {
    ...
    NSString *path = [NSString stringWithFormat:@"%@/%@", document, MBDBDefaultName];
    return path;
}
...
+ (instancetype)dbWithPath:(NSString *)path;
{
    ...
    MBDB *db = [[MBDB alloc] initWithPath:filePath];
...
{
    return [MBDB dbWithPath:[MBDB defaultDbPath]];
}
...
- (BOOL)executeQuery:(NSString *)query resultBlock:(MBDBExecuteCallBackBlock)block;
{
...
objc_property_t * st_class_copyPropertyList(Class cls, unsigned int *count)
{
    if (![cls isSubclassOfClass:[MBDBObject class]]) {
        return NULL;
    };
...
- (NSArray *)tableColumnsCache:(Class)cls {
    ...
    MBDB *MBDB = self;
    [self openDb];
    if (sqlite3_prepare_v2(MBDB->_sqlite3DB, [str UTF8String], -1, &stmt, NULL) == SQLITE_OK) {
    ...
    @synchronized(self){
        ...
        switch(type[0]) {
                ...
                //Handle different clases in here
            {
                ...
                if ([NSClassFromString(cls) isSubclassOfClass:[MBDBObject class]]) {
                    return DBText;
                }
            }
                break;
                    ...
                    for (NSString *key in dict) {
                        ...
                        if ([obj isKindOfClass:[NSString class]]) {
                            ...
                            if ([dbObj hasPrefix:DBChildPrefix]) {
                                ...
                                MBDBObject *child = (MBDBObject *)[NSClassFromString(clsName) dbObjectsWhere:where orderby:nil][0];
                ...
                //Handle different clases in here
            {
                ...
                if ([NSClassFromString(cls) isSubclassOfClass:[NSDictionary class]]) {
                    ...
                    for (NSString *key in dbValue) {
                        ...
                        if ([obj isKindOfClass:[MBDBObject class]]) {
                            MBDBObject *dbObject = (MBDBObject *)obj;
...
- (void)setMaxBusyRetryTimeInterval:(NSTimeInterval)timeout {
    ...
    if (timeout > 0) {
        sqlite3_busy_handler(_sqlite3DB, &MBDBBusyHandler, (__bridge void *)(self));
    }
    else {
...
- (NSMutableArray *)selectDbObjects:(Class)aClass condition:(NSString *)condition orderby:(NSString *)orderby
{
    ...
    if (sqlite3_prepare_v2(sqlite3DB, [selectstring UTF8String], -1, &stmt, NULL) == SQLITE_OK) {
        ...
        while (rc == SQLITE_ROW) {
            ...
            MBDBObject *obj = [[NSClassFromString(tableName) alloc] init];
...
- (BOOL)updateDbObject:(MBDBObject *)obj condition:(NSString *)condition
{
    //    @synchronized(self){
...
- (BOOL)insertDbObject:(MBDBObject *)obj;
{
    @synchronized(self){
...
- (BOOL)replaceDbObject:(MBDBObject *)obj;
{
    @synchronized(self){
...
- (BOOL)insertDbObject:(MBDBObject *)obj forced:(BOOL)forced
{
    @synchronized(self){
        if (![self isOpened]) {
            [self openDb];
...
- (void)createDbTable:(Class)aClass
{
    @synchronized(self){
        ...
        if ([self sqlite_tableExist:aClass]) {
            MBDBLog(@"数据库表%@已存在!", NSStringFromClass(aClass));
            return;
        }
...
- (BOOL)dbTable:(Class)aClass executeQuery:(NSString *)query callBackBlock:(void (^)(NSArray *resultArray))block;
{
    ...
    int ret = sqlite3_exec(db.sqlite3DB, [sql UTF8String], block ? MBDBExecuteBuMBSQLCallback : nil, (__bridge void *)(resultBlock), &errmsg);
...
int MBDBExecuteBuMBSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (NSString *)versionFilePath {
    ...
    NSString *file = [NSHomeDirectory() stringByAppendingFormat:@"/Documents/%@.MBDBversion", @(hash)];
    return file;
}
