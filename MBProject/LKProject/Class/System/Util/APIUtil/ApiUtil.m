...
+ (void)sendApiRequestWithArguments:(NSDictionary *)arguments requestURL:(NSString *)url requestMethod:(ApiRequestMethod)method MD5Encryption:(BOOL)MD5Encryption successBack:(ApiSuccessBack)successBack errorBack:(ApifailedErrorBack)errorBack {
    ...
    NSDictionary *encryptionDict = [[NSDictionary alloc] init];
    if (MD5Encryption) {
        if ([NSString validateNeedSaltValue:url]) {
            encryptionDict = [NSString generateArguments:arguments];
        }else {
            encryptionDict = [NSString generatePartArguments:arguments];
        }
    }else {
        encryptionDict = arguments;
    }
